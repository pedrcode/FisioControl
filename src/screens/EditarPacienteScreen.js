...
export default function EditarPacienteScreen({ route, navigation }) {
  ...
  const { getPaciente, editarPaciente } = useApp();
  ...
  const handleSalvar = () => {
    ...
    Alert.alert('✅ Salvo!', 'Cadastro atualizado com sucesso.', [
  ...
  return (
    <View style={[styles.safe, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>✏️ Editar Paciente</Text>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView style={styles.scroll} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 40 }]} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            ...
            <Text style={styles.label}>Observações / Evolução do Paciente</Text>
            <TextInput style={[styles.input, { minHeight: 120 }]} placeholder="Histórico clínico, evolução geral..." placeholderTextColor={COLORS.textLight} value={form.observacoes} onChangeText={(v) => handleChange('observacoes', v)} multiline textAlignVertical="top" />
